  ...
  while (1)
  {
    ...
    */
    ...
    if (type == SENSOR)
    {
      ...
      (void)Time(clock_server_tid); // discard timestamp; logged elsewhere
      if (!is_released)
      {
    ...
    if (type == SWITCH)
    {
      ...
      int switch_no = (unsigned char)msg[0];
    ...
    if (type == TRAIN)
    {
      ...
      // This is called by the worker task
      int train_no = (unsigned char)msg[0];
      char speed = msg[1];
...
void get_sensor_pushed(int track_server_tid, int *track, int tracks_max)
{
  ...
  Send(track_server_tid, buf, 4, (char *)track, sizeof(int) * tracks_max);
}
...
int getspeed_train(int track_server_tid, char train_no)
{
  ...
  int ret =0;
  Send(track_server_tid, buf, 4, (char *)&ret, 4);
  return ret;
  // return -1 if the task is declined
}
void deregister_train(int track_server_tid __attribute__((unused)), char train_no __attribute__((unused)))
{
  // the sensor node must be at a START NODE is to be called when a train task is created
  // to be completed
}
