...
void helper_send_message_to_server(int my_tid __attribute__((unused)), int track_server_tid, uint8_t s88_no_i, uint8_t changed, uint8_t is_b, uint8_t is_released)
{
  while (changed != 0)
  {
    char send_msg[4];
...
void marklin_worker()
{
  ...
  while (1)
  {
    ...
    io_TXIC_MARKLIN_server_pid = WhoIs(UART2_MARKLIN_SERVER);
    io_RXIC_MARKLIN_server_pid = io_TXIC_MARKLIN_server_pid;
    ...
    if(type == SWITCH){
      ...
      (void)read_many_s88(io_TXIC_MARKLIN_server_pid, io_RXIC_MARKLIN_server_pid, (uint8_t *)reta, (uint8_t *)retb);
...
void set_switch(int marklin_worker_tid, uint8_t sw_ind, char state){
    ...
    send_msg[0] = sw_ind;
    send_msg[1] = state;
    Send(marklin_worker_tid, send_msg, 4, (char *)&ret, 4);
}
