...
/* Where the prediction-error line lives. Out of the way of the existing UI. */
#define PRED_ROW 6
#define PRED_COL 1
    ...
    */
    ...
    if (type == SENSOR)
    {
      ...
      if (await_list.await_count)
      {
        ...
        /* 8-byte reply: 3 status bytes + 1 pad + 4-byte fire-time so the
           awaiter can measure prediction error without re-asking the clock. */
        char repmsg[8];
        repmsg[0] = s88_id;
        repmsg[1] = sensor_id;
        repmsg[2] = is_released;
        repmsg[3] = 0;
        *(uint32_t*)&repmsg[4] = time;
      ...
      /* Sensor prediction error display: only on pressed events for the
         active train, where active = most recently moving (speed>0). */
      if (!is_released && pred_active_train >= 0
          && train_speed[pred_active_train] > 0
          && train_speed[pred_active_train] < SPEED_MAX) {
        int spd = train_speed[pred_active_train];
        struct train_velocity *vel = &vel_list[pred_active_train][spd];

        if (pred_has && s88_id == pred_s88 && sensor_id == pred_sensor) {
          int err_ticks = (int)((int32_t)time - (int32_t)pred_time);
          int err_mm = (vel->dist * err_ticks) / vel->time;
          uart_printf(CONSOLE, "\033[%d;%dH\033[Kpred OK  hit=%c%d t-pred=%d ticks  v(dt)=%d mm",
                      PRED_ROW, PRED_COL, s88_id + 'A', sensor_id, err_ticks, err_mm);
        } else if (pred_has) {
          uart_printf(CONSOLE, "\033[%d;%dH\033[Kpred MISS  expected=%c%d got=%c%d",
                      PRED_ROW, PRED_COL,
                      pred_s88 + 'A', pred_sensor,
                      s88_id + 'A', sensor_id);
        }

        /* Project the next sensor along current switch states. */
        struct track_node *cur = trackmap[(int)s88_id][(int)sensor_id];
        if (cur != 0) {
          int dist = 0, isexit = 0;
          struct track_node *nxt = next_type_node(sw_states, NODE_SENSOR, cur, &dist, &isexit);
          if (nxt != 0 && nxt->type == NODE_SENSOR) {
            int next_s88 = nxt->name[0] - 'A';
            int next_no  = (int)atoi_64((char*)&nxt->name[1]);
            pred_s88    = next_s88;
            pred_sensor = next_no;
            pred_dist   = dist;
            pred_time   = time + (uint32_t)compute_time(dist, vel);
            pred_has    = 1;
            uart_printf(CONSOLE, "\033[%d;%dH\033[Kpredict next=%s dist=%dmm t=+%d ticks",
                        PRED_ROW + 1, PRED_COL, nxt->name, dist,
                        (int)(pred_time - time));
          } else {
            pred_has = 0;
            uart_printf(CONSOLE, "\033[%d;%dH\033[Kpredict next=(exit)            ",
                        PRED_ROW + 1, PRED_COL);
          }
        }
      }
    ...
    if (type == TRAIN)
    {
      ...
      char train_no = msg[0];
      char speed = msg[1];
      train_speed[(int)train_no] = speed;
      if (speed > 0) {
        pred_active_train = train_no;
        pred_has = 0;  /* invalidate stale prediction on speed change */
      } else if (train_no == pred_active_train) {
        pred_has = 0;
      }
      offset++;
      Reply(tid, msg, 0);
...
uint64_t await_sensor(int track_server_tid, char *ret)
{
  ...
  Send(track_server_tid, buf, 4, rep, 8);
  ret[0] = rep[0];   /* s88 id           */
  ret[1] = rep[1];   /* sensor number    */
  ret[2] = rep[2];   /* is_released      */
  uint32_t t = *(uint32_t*)&rep[4];
  return t;
}
