...
void enable_RX_and_TX() {
  // CONSOLE: enable RXIM (bit 4) and RTIM (bit 6). The FIFO is enabled
  // so RXIM only fires when the FIFO crosses its trigger level (2 bytes
  // by default) — RTIM is what delivers single keystrokes after a
  // receive-idle timeout. ExceptionASYNC's RIS_CONSOLE & RXIC branch
  // reads the byte and writes ICR_CONSOLE to ack both bits.
  uint32_t   line = 1;
  ...
  // Marklin TXIC disabled at startup: QEMU TX FIFO is always empty so TXIC fires
  // continuously and starves lower-priority tasks. Enable only when transmitting.
...
static void uart_format_print (size_t line, char *fmt, va_list va ) {
  ...
  while ( ( ch = *(fmt++) ) ) {
      ...
      case 'b':
        ...
        break;
      case 'c':
				/* va_arg promotes char to int; consume the argument value, not 'c' */
