...
int tc1ExecuteCommands(char *command, char **num, int command_part_count){
  ...
  if (strcmp_ret(command, "tcstd", 0)){
    int trainid = atoi_64(num[1]);
    (void)atoi_64(num[2]); // speed param not used by the (stubbed) tcstd
    if (trainid < 1 || trainid > 80){
      uart_printf(CONSOLE, "Invalid trainid\r\n");
      return 1;
    }
  ...
  if(strcmp_ret(command, "delaystop", 0)){
    int trainid = atoi_64(num[1]);
  ...
  if (strcmp_ret(command, "interruptstop", 0)){
    int trainid = atoi_64(num[1]);
    if (trainid < 1 || trainid > 80){
      uart_printf(CONSOLE, "Invalid trainid\r\n");
      return 1;
  ...
  if (strcmp_ret(command, "sensordelaystop", 0)){
    int trainid = atoi_64(num[1]);
