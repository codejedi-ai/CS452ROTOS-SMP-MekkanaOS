...
void command_shell()
{
  ...
  /* Block on Getc(io_RXIC_CONSOLE_server, CONSOLE) instead of polling.
     The notifier handles polling and feeds bytes to the server; we just
     ask for the next byte and sleep until one arrives. No busy-wait. */
  int rx_srv_tid = WhoIs("io_RXIC_CONSOLE_server");
  ...
  {
    char in = (char)Getc(rx_srv_tid, CONSOLE);
    if (in == 'a') in = 'A';
    else if (in == 'b') in = 'B';
    if (in == 'A' || in == 'B')
    {
  ...
  while (1)
  {
    c = (char)Getc(rx_srv_tid, CONSOLE);
