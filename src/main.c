...
int kmain(void *reg) {  
  ...
  /* Console (UART0) RX service. Same observer pattern as the Marklin ones
     but the notifier polls because the PL011 RX IRQ doesn't reach the GIC
     on QEMU raspi4b. Lets the shell block on Getc instead of busy-waiting. */
  tid = KernelCreate(0, io_RXIC_CONSOLE_server, 0);
  uart_printf(CONSOLE, "io_RXIC_CONSOLE_server tid: %d\r\n", tid);
  /* Same priority as clock_notifier so they share CPU via Yield/Delay tiebreaks.
     At lower priority the clock_notifier (which never blocks) starves this. */
  tid = KernelCreate(5, console_rx_notifier, 0);
  uart_printf(CONSOLE, "console_rx_notifier tid: %d\r\n", tid);
  #endif
