...
struct MinHeapState  READY_HEAP;
...
/*
 * Linear-scan ready queue.
 *
 * Was a binary min-heap; bubble-up/down had a corruption pattern the boot
 * self-tests surfaced (insert order vs extract order diverged). Swapped for
 * a flat-array linear-scan: insert appends, extract scans for the smallest.
 * With NUMPROCS=20 the O(n) per op is well below the 10ms tick budget.
 */
void insertKey_state_heap( struct MinHeapState *h, struct state k)
{
	k.time = ((uint64_t)get_timerHI() << 32) | (uint64_t)get_timerLO();
...
{
	if (h->size == 0) return (struct state){-1, -1};
	unsigned min_i = 0;
	for (unsigned i = 1; i < h->size; i++) {
		if (compare_state(h->harr[i], h->harr[min_i]) == 1) min_i = i;
	}
	struct state result = h->harr[min_i];
	h->size--;
	if (min_i != h->size) h->harr[min_i] = h->harr[h->size];
	return result;
}
...
{
	/* PID allocation always picks the lowest free slot:
	 *   - scan PROCS[0..NUMPROCS-1] for the first slot with pcpointer == NULL
	 *   - that gives us the lowest reusable PID (Kill leaves pcpointer NULL,
	 *     so reused slots automatically come back into the pool)
	 *   - if nothing is free we return -2 (capacity exhausted).
	 */
	int p = -1;
	for (int i = 0; i < NUMPROCS; i++) {
		if (PROCS[i].pcpointer == NULL) { p = i; break; }
	}
	if (p < 0) return -2;

	/* Zero everything before reinitializing so a recycled slot doesn't
	 * inherit stale saved registers, queues, or wait flags. */
	for (int r = 0; r < 31; r++) PROCS[p].registervalues[r] = 0;
	for (int q = 0; q < QUEUESIZE; q++) {
		PROCS[p].message_recieved[q].tid = 0;
		PROCS[p].message_recieved[q].msg = 0;
		PROCS[p].message_recieved[q].msglen = 0;
		PROCS[p].message_recieved[q].reply = 0;
		PROCS[p].message_recieved[q].replylen = 0;
	}
	PROCS[p].message_sent.tid = 0;
	PROCS[p].message_sent.msg = 0;
	PROCS[p].message_sent.msglen = 0;
	PROCS[p].message_sent.reply = 0;
	PROCS[p].message_sent.replylen = 0;
	PROCS[p].waiting_recieve_head = 0;
	PROCS[p].waiting_recieve_tail = 0;
	PROCS[p].queuesize = 0;
	PROCS[p].waiting_recieve = 0;
