...
#define RXIC_TYPE 4    /* same encoding as marklin RXIC byte                 */

/*
 * io_RXIC_CONSOLE_server
 *
 * Pairs incoming bytes from console_rx_notifier with pending Getc waiters.
 * Same observer-pattern shape as io_RXIC_MARKLIN_server, just on line
 * CONSOLE (= 1). When a byte arrives and a waiter is queued, Reply
 * unblocks the waiter with the byte in payload[2]; otherwise the byte
 * is buffered. Likewise, Getc calls that arrive before any byte block
 * in the call queue.
 */
void io_RXIC_CONSOLE_server(void)
{
	RegisterAs("io_RXIC_CONSOLE_server");
	struct fi_list interrupt_list = {0};
	struct fi_list call_list      = {0};
	int notifier_tid = -1;
	int tid;
	char recv[8];

	while (1) {
		if (notifier_tid <= 0) notifier_tid = WhoIs("console_rx_notifier");
		Receive(&tid, recv, 8);
		uint8_t type     = recv[0];
		uint8_t channel  = recv[1];
		uint8_t char_ch  = recv[2];

		/* Always release the notifier immediately so it can poll again. */
		if (tid == notifier_tid) {
			Reply(tid, recv, 0);
		}
		...
		/* Pair up oldest waiter with oldest interrupt. */
		while (call_list.size > 0 && interrupt_list.size > 0) {
			int ret_pid = call_list.call[call_list.begin].tid;
			recv[0] = interrupt_list.call[interrupt_list.begin].type;
			recv[1] = interrupt_list.call[interrupt_list.begin].channel;
			recv[2] = interrupt_list.call[interrupt_list.begin].char_ch;
			recv[3] = 0;
			Reply(ret_pid, recv, 8);
			call_list.call[call_list.begin].tid = 0;
			call_list.begin = (call_list.begin + 1) % QUEUELENGTH;
			call_list.size--;
			interrupt_list.begin = (interrupt_list.begin + 1) % QUEUELENGTH;
 ...
 * console_rx_notifier
 *
 * Polls UART0 RX FIFO (via uart_getc_queue) and forwards each byte to
 * io_RXIC_CONSOLE_server. Uses Delay(cs_tid, 1) instead of Yield so we
 * don't starve the rest of the system on the polling stride, same fix
 * as clock_notifier. This is the observer-pattern equivalent of an IRQ
 * handler -- shell code Getc()s and blocks, no busy-wait.
 */
void console_rx_notifier(void)
{
	RegisterAs("console_rx_notifier");
	int cs_tid     = -1;
	int rx_srv_tid = -1;
	while (cs_tid <= 0) {
		cs_tid = WhoIs("clock_server");
		if (cs_tid <= 0) Yield();
	}
	while (rx_srv_tid <= 0) {
		rx_srv_tid = WhoIs("io_RXIC_CONSOLE_server");
		if (rx_srv_tid <= 0) Yield();
	}
	char msg[8];
	char rep[8];
	while (1) {
		if (uart_getc_queue(CONSOLE)) {
			unsigned char ch = uart_getc_modified(CONSOLE);
			msg[0] = RXIC_TYPE;
			msg[1] = CONSOLE;
			msg[2] = (char)ch;
			msg[3] = 0;
			Send(rx_srv_tid, msg, 8, rep, 8);
		} else {
			Delay(cs_tid, 1);
		}
	}
